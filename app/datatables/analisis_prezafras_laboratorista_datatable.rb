...
class AnalisisPrezafrasLaboratoristaDatatable
...
private
  ...
  def data
    ...
    analisis_prezafras.each do |analisis_prezafra|
      ...
      a << analisis_prezafra.peso_muestra
      a << analisis_prezafra.peso_tallo
  ...
  def fetch_analisis_rsds                # use .includes for Left OUTER JOIN
    analisis_prezafras = AnalisisPrezafra.includes(:agente, :promotor, :laboratorista, :cania_variedad).
    ...
    if params[:sSearch].present?
      analisis_prezafras = analisis_prezafras.where(
                        ...
                        "CAST(peso_muestra AS TEXT) like :search OR " +
                        "CAST(peso_tallo AS TEXT) like :search OR " +
  ...
  def sort_column
    columns = ["((analisis_prezafras.fecha_ingreso is null) or (analisis_prezafras.fecha_analisis is null) or (analisis_prezafras.peso_muestra is null) or (analisis_prezafras.peso_tallo is null) or " +
               ...
               "analisis_prezafras.peso_muestra",
               "analisis_prezafras.peso_tallo",
